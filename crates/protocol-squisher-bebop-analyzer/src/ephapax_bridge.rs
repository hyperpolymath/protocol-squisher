...
//!
//! This module converts Bebop types analyzed from .bop files into ephapax IR types
...
mod tests {
    ...
    #[test]
    fn test_bebop_string_to_string() {
        let ctx = IRContext::new();
        let string_type = IrType::Primitive(IrPrim::String);
    ...
    #[test]
    fn test_bebop_array_to_vec() {
        use protocol_squisher_ir::ContainerType;

        let ctx = IRContext::new();
        let array_type = IrType::Container(ContainerType::Vec(Box::new(IrType::Primitive(
            IrPrim::I32,
        ))));
    ...
    #[test]
    fn test_bebop_guid_mapping() {
        let ctx = IRContext::new();
        let guid_type = IrType::Primitive(IrPrim::Uuid);

        // Bebop GUID should map to Uuid in IR (though ephapax doesn't support it yet)
        let ephapax_prim = to_ephapax_primitive(&guid_type);
        assert_eq!(ephapax_prim, None); // Not yet supported in ephapax
    }

    #[test]
    fn test_bebop_date_mapping() {
        let ctx = IRContext::new();
        let date_type = IrType::Primitive(IrPrim::DateTime);

        // Bebop date should map to DateTime in IR (though ephapax doesn't support it yet)
        let ephapax_prim = to_ephapax_primitive(&date_type);
        assert_eq!(ephapax_prim, None); // Not yet supported in ephapax
    }
