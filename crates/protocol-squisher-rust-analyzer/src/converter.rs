    ...
    match ty {
        ...
        syn::Type::Tuple(type_tuple) => {
            if type_tuple.elems.is_empty() {
                ...
                let types: Result<Vec<_>, _> = type_tuple.elems.iter()
                    .map(convert_type)
                    .collect();
        ...
        syn::Type::Array(type_array) => {
            ...
            Ok(IrType::Container(ContainerType::Array(Box::new(inner), len)))
        }
        ...
        syn::Type::Paren(type_paren) => {
            convert_type(&type_paren.elem)
        }
        syn::Type::Group(type_group) => {
            convert_type(&type_group.elem)
        }
        syn::Type::Never(_) => {
            Ok(IrType::Special(SpecialType::Never))
        }
...
fn convert_path_type(type_path: &syn::TypePath) -> Result<IrType, AnalyzerError> {
    ...
    match ident.as_str() {
        // Option
        "Option" => {
            let inner = generic_args.first()
                .ok_or_else(|| AnalyzerError::ParseError("Option requires type argument".to_string()))?;
            Ok(IrType::Container(ContainerType::Option(Box::new(convert_type(inner)?))))
        }

        // Vec / vectors
        "Vec" | "VecDeque" => {
            let inner = generic_args.first()
                .ok_or_else(|| AnalyzerError::ParseError("Vec requires type argument".to_string()))?;
            Ok(IrType::Container(ContainerType::Vec(Box::new(convert_type(inner)?))))
        }

        // Sets
        "HashSet" | "BTreeSet" | "IndexSet" => {
            let inner = generic_args.first()
                .ok_or_else(|| AnalyzerError::ParseError("Set requires type argument".to_string()))?;
            Ok(IrType::Container(ContainerType::Set(Box::new(convert_type(inner)?))))
        }

        // Maps
        "HashMap" | "BTreeMap" | "IndexMap" => {
            if generic_args.len() < 2 {
                return Err(AnalyzerError::ParseError("Map requires key and value type arguments".to_string()));
            }
            let key = convert_type(&generic_args[0])?;
            let value = convert_type(&generic_args[1])?;
            Ok(IrType::Container(ContainerType::Map(Box::new(key), Box::new(value))))
        }

        // Result
        "Result" => {
            if generic_args.len() < 2 {
                return Err(AnalyzerError::ParseError("Result requires Ok and Err type arguments".to_string()));
            }
            let ok = convert_type(&generic_args[0])?;
            let err = convert_type(&generic_args[1])?;
            Ok(IrType::Container(ContainerType::Result(Box::new(ok), Box::new(err))))
        }

        // Box is transparent for serialization
        "Box" | "Rc" | "Arc" | "Cow" => {
            let inner = generic_args.first()
                .ok_or_else(|| AnalyzerError::ParseError("Smart pointer requires type argument".to_string()))?;
            convert_type(inner)
        }
        ...
        "Cell" | "RefCell" | "Mutex" | "RwLock" => {
            let inner = generic_args.first()
                .ok_or_else(|| AnalyzerError::ParseError("Cell type requires type argument".to_string()))?;
            convert_type(inner)
        }

        // JSON value
        "Value" if is_serde_json_path(path) => {
...
mod tests {
    ...
    fn test_option_type() {
        ...
        if let IrType::Container(ContainerType::Option(inner)) = ty {
            assert!(matches!(*inner, IrType::Primitive(PrimitiveType::I32)));
        } else {
    ...
    fn test_vec_type() {
        ...
        if let IrType::Container(ContainerType::Vec(inner)) = ty {
    ...
    fn test_hashmap_type() {
        ...
        if let IrType::Container(ContainerType::Map(key, value)) = ty {
            assert!(matches!(*key, IrType::Primitive(PrimitiveType::String)));
            assert!(matches!(*value, IrType::Primitive(PrimitiveType::I32)));
        } else {
    ...
    fn test_nested_types() {
        ...
        if let IrType::Container(ContainerType::Option(inner)) = ty {
            if let IrType::Container(ContainerType::Vec(inner2)) = *inner {
                assert!(matches!(*inner2, IrType::Primitive(PrimitiveType::String)));
            } else {
                panic!("Expected Vec inside Option");
            }
        } else {
            panic!("Expected Option container");
        }
    ...
    fn test_tuple_type() {
        ...
        if let IrType::Container(ContainerType::Tuple(types)) = ty {
