...
//!
//! This module converts Cap'n Proto types analyzed from .capnp files into ephapax IR types
//! for transport class analysis.
//!
//! # Cap'n Proto Zero-Copy Characteristics
//!
//! Cap'n Proto is designed for zero-copy serialization:
//! - Primitive types (Bool, Int*, UInt*, Float*) are inline → Concorde
//! - Text and Data are POINTERS → Economy (requires dereference)
//! - List(T) is a POINTER → Economy (header indirection)
//! - Structs are pointer-based but fields can be zero-copy → Concorde
//!
...
fn analyze_container_compatibility(
    ...
    match (source, target) {
        ...
        // Vec<T> analysis (Cap'n Proto List(T))
        (ContainerType::Vec(source_inner), ContainerType::Vec(target_inner)) => {
            // Vec has pointer overhead in Cap'n Proto, but propagate inner class
...
mod tests {
    ...
    #[test]
    fn test_capnp_list_to_vec() {
        use protocol_squisher_ir::ContainerType;

        let ctx = IRContext::new();
        let list_type = IrType::Container(ContainerType::Vec(Box::new(IrType::Primitive(
            IrPrim::I32,
        ))));
        let vec_type = IrType::Container(ContainerType::Vec(Box::new(IrType::Primitive(
            IrPrim::I32,
        ))));
    ...
    #[test]
    fn test_capnp_void_to_unit() {
        use protocol_squisher_ir::SpecialType;
    ...
    #[test]
    fn test_capnp_data_to_bytes() {
        use protocol_squisher_ir::ContainerType;

        let ctx = IRContext::new();
        let data_type = IrType::Container(ContainerType::Vec(Box::new(IrType::Primitive(
            IrPrim::U8,
        ))));
