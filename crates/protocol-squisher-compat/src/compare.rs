...
pub fn compare_types(source: &IrType, target: &IrType, path: &str) -> TypeComparison {
    match (source, target) {
        ...
        (IrType::Primitive(src), IrType::Primitive(tgt)) => {
            compare_primitives(src, tgt, path)
        }

        // Container comparisons
        (IrType::Container(src), IrType::Container(tgt)) => {
            compare_containers(src, tgt, path)
        }

        // Special type comparisons
        (IrType::Special(src), IrType::Special(tgt)) => {
            compare_specials(src, tgt, path)
        }
        ...
        (IrType::Primitive(_), IrType::Container(ContainerType::Option(inner))) => {
            ...
            inner_cmp.combine(TypeComparison::new(TransportClass::BusinessClass, path)
                .with_loss(ConversionLoss {
        ...
        (IrType::Container(ContainerType::Option(inner)), IrType::Primitive(_)) => {
            ...
            inner_cmp.combine(TypeComparison::new(TransportClass::Economy, path)
                .with_loss(ConversionLoss {
                    kind: LossKind::NullabilityChanged,
                    path: path.to_string(),
        ...
        (IrType::Special(SpecialType::Json), _) => {
            TypeComparison::new(TransportClass::Economy, path)
                .with_loss(ConversionLoss {
        ...
        // Incompatible types
        _ => TypeComparison::new(TransportClass::Incompatible, path)
            .with_loss(ConversionLoss {
                kind: LossKind::TypeCoercion,
                path: path.to_string(),
...
fn compare_primitives(source: &PrimitiveType, target: &PrimitiveType, path: &str) -> TypeComparison {
    use PrimitiveType::*;
    ...
    match (source, target) {
        ...
        (F32 | F64, I8 | I16 | I32 | I64 | I128 | U8 | U16 | U32 | U64 | U128) => {
            TypeComparison::new(TransportClass::Wheelbarrow, path)
                .with_loss(ConversionLoss {
                    kind: LossKind::PrecisionLoss,
                    path: path.to_string(),
...
fn compare_containers(source: &ContainerType, target: &ContainerType, path: &str) -> TypeComparison {
    match (source, target) {
        // Option comparisons
        ...
        (ContainerType::Vec(src), ContainerType::Array(tgt, len)) => {
            ...
            inner.combine(TypeComparison::new(TransportClass::Economy, path)
                .with_loss(ConversionLoss {
                    kind: LossKind::RangeLoss,
                    path: path.to_string(),
        ...
        (ContainerType::Set(src), ContainerType::Vec(tgt)) => {
            let inner = compare_types(src, tgt, &format!("{path}[]"));
            inner.combine(TypeComparison::new(TransportClass::BusinessClass, path)
                .with_loss(ConversionLoss {
        ...
        (ContainerType::Tuple(src), ContainerType::Tuple(tgt)) => {
            if src.len() != tgt.len() {
                return TypeComparison::new(TransportClass::Incompatible, path)
                    .with_loss(ConversionLoss {
                        kind: LossKind::TypeCoercion,
                        path: path.to_string(),
                        description: format!("Tuple length mismatch: {} vs {}", src.len(), tgt.len()),
                        severity: LossSeverity::Critical,
                    });
            }
...
fn compare_specials(source: &SpecialType, target: &SpecialType, path: &str) -> TypeComparison {
    ...
    match (source, target) {
        (SpecialType::Unit, SpecialType::Never) |
        (SpecialType::Never, SpecialType::Unit) => {
            TypeComparison::new(TransportClass::BusinessClass, path)
        }
...
mod tests {
    ...
    fn test_option_inner() {
        let src = IrType::Container(ContainerType::Option(
            Box::new(IrType::Primitive(PrimitiveType::I32))
        ));
        let tgt = IrType::Container(ContainerType::Option(
    ...
    #[test]
    fn test_vec_to_set() {
        let src = IrType::Container(ContainerType::Vec(
            Box::new(IrType::Primitive(PrimitiveType::String))
        ));
        let tgt = IrType::Container(ContainerType::Set(
            Box::new(IrType::Primitive(PrimitiveType::String))
        ));
        let cmp = compare_types(&src, &tgt, "test");
        assert_eq!(cmp.class, TransportClass::Economy);
