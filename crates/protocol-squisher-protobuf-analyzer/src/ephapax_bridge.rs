...
//!
//! This module converts Protobuf types analyzed from .proto files into ephapax IR types
    ...
    match (source, target) {
        ...
        (IrType::Special(_), IrType::Special(_)) => {
            let source_prim = to_ephapax_primitive(source)
                .ok_or_else(|| AnalyzerError::UnsupportedFeature(
...
mod tests {
    ...
    #[test]
    fn test_protobuf_double_to_f64() {
        let ctx = IRContext::new();
    ...
    #[test]
    fn test_protobuf_bytes_to_vec_u8() {
        let ctx = IRContext::new();

        // In protobuf, bytes is typically represented as Vec<u8>
        let bytes_as_vec = IrType::Container(
            protocol_squisher_ir::ContainerType::Vec(Box::new(IrType::Primitive(IrPrim::U8)))
        );
        let vec_u8 = IrType::Container(
            protocol_squisher_ir::ContainerType::Vec(Box::new(IrType::Primitive(IrPrim::U8)))
        );
