...
*Implemented:* - `+Type::Vec(Box<Type>)+` for vector types -
...
`+[]+` (default to `+Vec<i32>+`) - Mixed element types: `+Vec<i32>+`,
`+Vec<String>+`, `+Vec<bool>+` - Type-safe indexing - Linear type
...
`+Vec<T>+`, nested structs - Linear type enforcement (Struct not Copy) -
...
Result<T, E> type - Error handling for file operations - Estimate: 2-3
days
