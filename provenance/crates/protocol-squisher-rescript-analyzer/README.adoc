...
|`+Js.Dict.t<V>+` |`+Map<String, V>+` |`+HashMap<String, V>+` |Concorde
(if V is)
|===
...
`+i64+` - ReScript `+string+` → Rust `+String+` - ReScript `+array<T>+`
→ Rust `+Vec<T>+` (if T is Concorde)
...
(`+external fetch: string => promise<response>+`) - Module-qualified
