...
Becomes: - `+Config+` struct with `+settings: Map<String, String>+`
field
...
. *Container handling*: Wraps repeated fields in `+Vec<T>+`, maps in
`+Map<K, V>+`
. *Optional fields*: Wraps in `+Option<T>+` based on syntax and label
