...
The `protocol-squisher` CLI provides four main commands:
...
protocol-squisher check --rust src/lib.rs --python models.py --quiet
----
...
|0
|Schemas are compatible (Concorde or Business class)

|1
|Schemas have significant issues (Wheelbarrow class)

|2
|Analysis error (invalid schema, file not found, etc.)
|===
...
|`--quiet`
|Only output errors (for CI/CD scripts)

|`--fail-on-business`
|Exit with code 1 if any Business-class fields (strict mode)

|`--fail-on-wheelbarrow`
|Exit with code 1 if any Wheelbarrow-class fields (default)
|===
...
protocol-squisher optimize --rust src/lib.rs --python models.py --min-impact high
----
...
|`--min-impact <LEVEL>`
|Only show suggestions with given impact (low/medium/high)

|`--sort-by <FIELD>`
|Sort suggestions by: impact (default), name, class
|===
...
|`--no-warnings`
|Suppress WARNING comments for Wheelbarrow fields
|===
...
This will fail the CI build if any Wheelbarrow-class fields are detected.
